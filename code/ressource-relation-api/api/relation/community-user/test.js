  ...
  describe("Post /relations", () => {
    ...
    describe("Get /relations/x/y", () => {
      it("should Get specific relations", done => {
        chai
          .request(url + relationPort)
          .get("/communityUserRelations/2/8")
          .end((err, res) => {
            if (err) done(err);
            expect(res).to.have.status(200);
            expect(res.body).to.be.an("object");
      ...
      it("should return 404 on inexistant community", done => {
        chai
          .request(url + relationPort)
          .get("/communityUserRelations/20/8")
          .end((err, res) => {
            if (err) done(err);
      ...
      it("should return 404 on inexistant resource", done => {
        chai
          .request(url + relationPort)
          .get("/communityUserRelations/2/22")
          .end((err, res) => {
            if (err) done(err);
            expect(res).to.have.status(404);
            expect(res.body).to.deep.equal({});
      ...
      it("should return 404 on invalid url", done => {
        chai
          .request(url + relationPort)
          .get("/communityUserRelations/2/null")
          .end((err, res) => {
            if (err) done(err);
            expect(res).to.have.status(404);
            expect(res.body).to.deep.equal({});
    ...
    describe("Put /relations/x/y", () => {
      it("should Put specific values in relations", done => {
        chai
          .request(url + relationPort)
          .put("/communityUserRelations/8/2")
          .send({
            CommunityId: 8,
            UserId: 2
          })
    ...
    describe("Delete /relations/x/y", () => {
      it("should Delete specific relations", done => {
        chai
          .request(url + relationPort)
          .delete("/communityUserRelations/8/2")
          .then(res => {
            expect(res).to.have.status(200);
            expect(res.body).to.be.an("object");
            expect(res.body).to.containSubset({
          ...
          .then(function(res) {
            chai
              .request(url + relationPort)
              .get("/communityUserRelations/8/2")
              .end((err, res) => {
                if (err) done(err);
                expect(res).to.have.status(404);
                done();
      ...
      it("should return 404 on inexisting realtion Delete specific relations", done => {
        chai
          .request(url + relationPort)
          .delete("/communityUserRelations/2/2")
          .end((err, res) => {
            if (err) done(err);
            expect(res).to.have.status(404);
            expect(res.body).to.deep.equal({});
