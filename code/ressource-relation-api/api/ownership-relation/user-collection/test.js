...
describe("ownership-relation-user-collection", async () => {
  ...
  describe("Get /ownershipRelations", () => {
    it("should Get ownershipRelations", done => {
      chai
        ...
        .get("/userCollectionOwnershipRelations")
        .end((err, res) => {
          if (err) done(err);
  ...
  describe("Post /ownershipRelations", () => {
    it("should Post ownershipRelations", done => {
      chai
        .request(url + ownershipRelationPort)
        .post("/userCollectionOwnershipRelations")
        .send({
          UserId: 6,
          CollectionId: 3,
          OwnershipTypeId: 1
  ...
  describe("Get /ownershipRelations/x/y", () => {
    it("should Get specific relations", done => {
      chai
        .request(url + ownershipRelationPort)
        .get("/userCollectionOwnershipRelations/2/8")
        .end((err, res) => {
          if (err) done(err);
          expect(res).to.have.status(200);
          expect(res.body).to.be.an("object");
    ...
    it("should return 404 on inexistant user", done => {
      chai
        .request(url + ownershipRelationPort)
        .get("/userCollectionOwnershipRelations/20/8")
        .end((err, res) => {
          if (err) done(err);
    ...
    it("should return 404 on inexistant resource", done => {
      chai
        .request(url + ownershipRelationPort)
        .get("/userCollectionOwnershipRelations/2/22")
        .end((err, res) => {
          if (err) done(err);
          expect(res).to.have.status(404);
          expect(res.body).to.deep.equal({});
    ...
    it("should return 404 on invalid url", done => {
      chai
        .request(url + ownershipRelationPort)
        .get("/userCollectionOwnershipRelations/2/null")
        .end((err, res) => {
          if (err) done(err);
          expect(res).to.have.status(404);
          expect(res.body).to.deep.equal({});
  ...
  describe("Put /ownershipRelations/x/y", () => {
    it("should Put specific values in relations", done => {
      chai
        .request(url + ownershipRelationPort)
        .put("/userCollectionOwnershipRelations/8/2")
        .send({
          UserId: 8,
          CollectionId: 2,
          OwnershipTypeId: 5
  ...
  describe("Delete /ownershipRelations/x/y", () => {
    it("should Delete specific relations", done => {
      chai
        .request(url + ownershipRelationPort)
        .delete("/userCollectionOwnershipRelations/8/2")
        .then(res => {
          expect(res).to.have.status(200);
          expect(res.body).to.be.an("object");
          expect(res.body).to.containSubset({
        ...
        .then(function(res) {
          chai
            .request(url + ownershipRelationPort)
            .get("/userCollectionOwnershipRelations/8/2")
            .end((err, res) => {
              if (err) done(err);
              expect(res).to.have.status(404);
              done();
    ...
    it("should return 404 on inexisting realtion Delete specific relations", done => {
      chai
        .request(url + ownershipRelationPort)
        .delete("/userCollectionOwnershipRelations/2/2")
        .end((err, res) => {
          if (err) done(err);
          expect(res).to.have.status(404);
          expect(res.body).to.deep.equal({});
